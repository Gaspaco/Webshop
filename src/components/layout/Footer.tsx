...
export default function Footer() {
  ...
  onMount(async () => {
    ...
    try {
      ...
      if (!content) return;
      setSocials(current => ({
        tiktok: safeSocialUrl(content.socialTiktok, current.tiktok),
        youtube: safeSocialUrl(content.socialYoutube, current.youtube),
        discord: safeSocialUrl(content.socialDiscord, current.discord),
        instagram: safeSocialUrl(content.socialInstagram, current.instagram),
      }));
    } catch {
      // Keep the public network pages as safe fallbacks.
    }
  });
      ...
      <div class={`${styles.wide} ${styles.top}`}>
        <div class={styles.brandCol}>
          ...
          <nav class={styles.socials} aria-label="Social media">
            ...
            <a href={socials().discord} target="_blank" rel="noreferrer" aria-label="Discord">
              <svg viewBox="0 0 24 24" aria-hidden="true">
